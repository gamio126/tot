...
<p>JQuery Dom select은 tagName, id, class 가 없더라도 찾아가는게 가능함. parent(1)
	, childlen(1+), sibling (siblings:1+ , prev:1 , next:1 ) 으로도 가능</p>
<hr />
		...
		<c:forTokens items="물,식량,선원,화약" delims="," var="s">
			<li><b>${s }</b>
				<button class="mbt">-</button>
					<input type="number" id="cnt" value="1" style="width: 45px;" />
				<button class="pbt">+</button></li>
		</c:forTokens>
	</ul>
...
<script>
	$(".pbt").click(function(){
		if($(".pbt").prev().prev().prev().eq(0).find("b")){
